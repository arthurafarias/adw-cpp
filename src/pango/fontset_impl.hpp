...
void FontsetClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.foreach) methods->foreach = (decltype (methods->foreach)) gi::detail::method_wrapper<self, void (*) (Pango::FontsetForeachFunc func), gi::transfer_full_t, std::tuple<detail::arg_info<gi::transfer_none_t, false, Pango::internal::FontsetForeachFunc_CB_Trait, detail::args_index<0, 1>>>, void  (Pango::FontsetForeachFunc::cfunction_type func, void* data)>::wrapper<&self::foreach_>;
  if (init_data.get_font) methods->get_font = (decltype (methods->get_font)) gi::detail::method_wrapper<self, Pango::Font (*) (guint wc), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::get_font_>;
  if (init_data.get_language) methods->get_language = (decltype (methods->get_language)) gi::detail::method_wrapper<self, Pango::Language (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_language_>;
  if (init_data.get_metrics) methods->get_metrics = (decltype (methods->get_metrics)) gi::detail::method_wrapper<self, Pango::FontMetrics (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_metrics_>;
}
