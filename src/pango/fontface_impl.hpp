...
void FontFaceClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_face_name) methods->get_face_name = (decltype (methods->get_face_name)) gi::detail::method_wrapper<self, gi::cstring_v (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_face_name_>;
  if (init_data.get_family) methods->get_family = (decltype (methods->get_family)) gi::detail::method_wrapper<self, Pango::FontFamily (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_family_>;
  if (init_data.is_synthesized) methods->is_synthesized = (decltype (methods->is_synthesized)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::is_synthesized_>;
  if (init_data.list_sizes) methods->list_sizes = (decltype (methods->list_sizes)) gi::detail::method_wrapper<self, void (*) (gi::Collection<gi::DSpan, gint, gi::transfer_full_t> & sizes), gi::transfer_full_t, std::tuple<detail::arg_info<gi::transfer_full_t, false, void, detail::args_index<0, 1>>>, void  (gint** sizes, gint* n_sizes)>::wrapper<&self::list_sizes_>;
}
