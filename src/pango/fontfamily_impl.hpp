...
void FontFamilyClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_face) methods->get_face = (decltype (methods->get_face)) gi::detail::method_wrapper<self, Pango::FontFace (*) (const gi::cstring_v name), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::get_face_>;
  ...
  if (init_data.is_monospace) methods->is_monospace = (decltype (methods->is_monospace)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::is_monospace_>;
  if (init_data.is_variable) methods->is_variable = (decltype (methods->is_variable)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::is_variable_>;
  if (init_data.list_faces) methods->list_faces = (decltype (methods->list_faces)) gi::detail::method_wrapper<self, void (*) (gi::Collection<gi::DSpan, ::PangoFontFace*, gi::transfer_container_t> & faces), gi::transfer_full_t, std::tuple<detail::arg_info<gi::transfer_container_t, false, void, detail::args_index<0, 1>>>, void  (::PangoFontFace*** faces, gint* n_faces)>::wrapper<&self::list_faces_>;
}
