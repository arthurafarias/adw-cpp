...
// PangoFontFace* /*none,nullable*/ pango_font_get_face (PangoFont* font /*none*/);
// ::PangoFontFace* /*none,nullable*/ pango_font_get_face (::PangoFont* font /*none*/);
...
void FontClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.describe) methods->describe = (decltype (methods->describe)) gi::detail::method_wrapper<self, Pango::FontDescription (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::describe_>;
  if (init_data.describe_absolute) methods->describe_absolute = (decltype (methods->describe_absolute)) gi::detail::method_wrapper<self, Pango::FontDescription (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::describe_absolute_>;
  if (init_data.get_coverage) methods->get_coverage = (decltype (methods->get_coverage)) gi::detail::method_wrapper<self, Pango::Coverage (*) (Pango::Language_Ref language), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::get_coverage_>;
  if (init_data.get_font_map) methods->get_font_map = (decltype (methods->get_font_map)) gi::detail::method_wrapper<self, Pango::FontMap (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_font_map_>;
  if (init_data.get_glyph_extents) methods->get_glyph_extents = (decltype (methods->get_glyph_extents)) gi::detail::method_wrapper<self, void (*) (::PangoGlyph glyph, Pango::Rectangle_Ref ink_rect, Pango::Rectangle_Ref logical_rect), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::get_glyph_extents_>;
  if (init_data.get_metrics) methods->get_metrics = (decltype (methods->get_metrics)) gi::detail::method_wrapper<self, Pango::FontMetrics (*) (Pango::Language_Ref language), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::get_metrics_>;
}
