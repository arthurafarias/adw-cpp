...
// ::GBytes* /*full*/ g_bytes_new_from_bytes (::GBytes* bytes /*none*/, gsize offset, gsize length);
GLib::Bytes base::BytesBase::new_from_bytes (GLib::Bytes_Ref bytes, gsize offset, gsize length) noexcept
{
  ...
  auto _temp_ret = call_wrap_v ((::GBytes*) (bytes_to_c), (gsize) (offset_to_c), (gsize) (length_to_c));
  return gi::wrap (_temp_ret, gi::transfer_full);
}
...
GLib::Bytes base::BytesBase::new_with_free_func (const guint8 * data, gsize size, GLib::DestroyNotify free_func) noexcept
{
  typedef ::GBytes* (*call_wrap_t) (const guint8* data, gsize size, GLib::DestroyNotify::cfunction_type free_func, void* user_data);
  call_wrap_t call_wrap_v = (call_wrap_t) g_bytes_new_with_free_func;
  auto free_func_wrap_ = free_func ? unwrap (std::move (free_func), gi::scope_async) : nullptr;
  auto data_to_c = data;
  auto _temp_ret = call_wrap_v ((const guint8*) (data_to_c), (gsize) (size), (GLib::DestroyNotify::cfunction_type) (free_func_wrap_ ? &free_func_wrap_->wrapper : nullptr), (void*) (free_func_wrap_));
  return gi::wrap (_temp_ret, gi::transfer_full);
}
