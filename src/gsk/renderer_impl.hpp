...
// gboolean gsk_renderer_realize_for_display (GskRenderer* renderer /*none*/, GdkDisplay* display /*none*/, GError ** error);
// gboolean gsk_renderer_realize_for_display (::GskRenderer* renderer /*none*/, ::GdkDisplay* display /*none*/, GError ** error);
bool base::RendererBase::realize_for_display (Gdk::Display display)
{
  typedef gboolean (*call_wrap_t) (::GskRenderer* renderer, ::GdkDisplay* display, GError ** error);
  call_wrap_t call_wrap_v = (call_wrap_t) ::gsk_renderer_realize_for_display;
  auto display_to_c = gi::unwrap (display, gi::transfer_none);
  GError *error = NULL;
  auto _temp_ret = call_wrap_v ((::GskRenderer*) (gobj_()), (::GdkDisplay*) (display_to_c), &error);
  gi::check_error (error);
  return _temp_ret;
}
bool base::RendererBase::realize_for_display (Gdk::Display display, GLib::Error * _error) noexcept
{
  typedef gboolean (*call_wrap_t) (::GskRenderer* renderer, ::GdkDisplay* display, GError ** error);
  call_wrap_t call_wrap_v = (call_wrap_t) ::gsk_renderer_realize_for_display;
  auto display_to_c = gi::unwrap (display, gi::transfer_none);
  GError* _error_o {};
  auto _temp_ret = call_wrap_v ((::GskRenderer*) (gobj_()), (::GdkDisplay*) (display_to_c), _error ? &_error_o : nullptr);
  if (_error) *_error = gi::wrap (_error_o, gi::transfer_full);
  return _temp_ret;
}
