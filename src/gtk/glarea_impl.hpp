...
void GLAreaClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.render) methods->render = (decltype (methods->render)) gi::detail::method_wrapper<self, bool (*) (Gdk::GLContext context), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::render_>;
  if (init_data.resize) methods->resize = (decltype (methods->resize)) gi::detail::method_wrapper<self, void (*) (gint width, gint height), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::resize_>;
}
