...
void CellEditableIfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.editing_done) methods->editing_done = (decltype (methods->editing_done)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::editing_done_>;
  if (init_data.remove_widget) methods->remove_widget = (decltype (methods->remove_widget)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::remove_widget_>;
}
