...
void ShortcutManagerInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.add_controller) methods->add_controller = (decltype (methods->add_controller)) gi::detail::method_wrapper<self, void (*) (Gtk::ShortcutController controller), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::add_controller_>;
  if (init_data.remove_controller) methods->remove_controller = (decltype (methods->remove_controller)) gi::detail::method_wrapper<self, void (*) (Gtk::ShortcutController controller), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::remove_controller_>;
}
