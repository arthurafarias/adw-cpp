...
void FontChooserIfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.font_activated) methods->font_activated = (decltype (methods->font_activated)) gi::detail::method_wrapper<self, void (*) (const gi::cstring_v fontname), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::font_activated_>;
  if (init_data.get_font_face) methods->get_font_face = (decltype (methods->get_font_face)) gi::detail::method_wrapper<self, Pango::FontFace (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_font_face_>;
  if (init_data.get_font_family) methods->get_font_family = (decltype (methods->get_font_family)) gi::detail::method_wrapper<self, Pango::FontFamily (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_font_family_>;
  if (init_data.get_font_map) methods->get_font_map = (decltype (methods->get_font_map)) gi::detail::method_wrapper<self, Pango::FontMap (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_font_map_>;
  if (init_data.get_font_size) methods->get_font_size = (decltype (methods->get_font_size)) gi::detail::method_wrapper<self, gint (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_font_size_>;
  if (init_data.set_filter_func) methods->set_filter_func = (decltype (methods->set_filter_func)) gi::detail::method_wrapper<self, void (*) (Gtk::FontFilterFunc filter), gi::transfer_full_t, std::tuple<detail::arg_info<gi::transfer_none_t, false, Gtk::internal::FontFilterFunc_CB_Trait, detail::args_index<0, 1, 2>>>, void  (Gtk::FontFilterFunc::cfunction_type filter, void* user_data, GLib::DestroyNotify::cfunction_type destroy)>::wrapper<&self::set_filter_func_>;
  if (init_data.set_font_map) methods->set_font_map = (decltype (methods->set_font_map)) gi::detail::method_wrapper<self, void (*) (Pango::FontMap fontmap), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::set_font_map_>;
}
