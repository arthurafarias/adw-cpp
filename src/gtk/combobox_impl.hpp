...
void ComboBoxClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.format_entry_text) methods->format_entry_text = (decltype (methods->format_entry_text)) gi::detail::method_wrapper<self, gi::cstring (*) (const gi::cstring_v path), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::format_entry_text_>;
}
