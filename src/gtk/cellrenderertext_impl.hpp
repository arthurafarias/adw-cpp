...
void CellRendererTextClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.edited) methods->edited = (decltype (methods->edited)) gi::detail::method_wrapper<self, void (*) (const gi::cstring_v path, const gi::cstring_v new_text), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::edited_>;
}
