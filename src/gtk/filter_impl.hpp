...
void FilterClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_strictness) methods->get_strictness = (decltype (methods->get_strictness)) gi::detail::method_wrapper<self, Gtk::FilterMatch (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_strictness_>;
  if (init_data.match) methods->match = (decltype (methods->match)) gi::detail::method_wrapper<self, bool (*) (GObject::Object item), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::match_>;
}
