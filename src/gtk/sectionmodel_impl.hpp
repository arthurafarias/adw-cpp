...
void SectionModelInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.get_section) methods->get_section = (decltype (methods->get_section)) gi::detail::method_wrapper<self, void (*) (guint position, guint & out_start, guint & out_end), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_full_t, gi::transfer_full_t>>::wrapper<&self::get_section_>;
}
