...
void MediaStreamClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.pause) methods->pause = (decltype (methods->pause)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::pause_>;
  if (init_data.play) methods->play = (decltype (methods->play)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::play_>;
  if (init_data.realize) methods->realize = (decltype (methods->realize)) gi::detail::method_wrapper<self, void (*) (Gdk::Surface surface), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::realize_>;
  if (init_data.seek) methods->seek = (decltype (methods->seek)) gi::detail::method_wrapper<self, void (*) (gint64 timestamp), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::seek_>;
  if (init_data.unrealize) methods->unrealize = (decltype (methods->unrealize)) gi::detail::method_wrapper<self, void (*) (Gdk::Surface surface), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::unrealize_>;
  if (init_data.update_audio) methods->update_audio = (decltype (methods->update_audio)) gi::detail::method_wrapper<self, void (*) (gboolean muted, gdouble volume), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::update_audio_>;
}
