...
void PollableInputStreamInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.is_readable) methods->is_readable = (decltype (methods->is_readable)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::is_readable_>;
  if (init_data.read_nonblocking) methods->read_nonblocking = (decltype (methods->read_nonblocking)) gi::detail::method_wrapper<self, gssize (*) (guint8 * buffer, gsize count, GLib::Error * _error), gi::transfer_none_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_full_t>>::wrapper<&self::read_nonblocking_>;
}
