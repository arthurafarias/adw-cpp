...
void DBusInterfaceSkeletonClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.flush) methods->flush = (decltype (methods->flush)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::flush_>;
  if (init_data.g_authorize_method) methods->g_authorize_method = (decltype (methods->g_authorize_method)) gi::detail::method_wrapper<self, bool (*) (Gio::DBusMethodInvocation invocation), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::g_authorize_method_>;
  if (init_data.get_info) methods->get_info = (decltype (methods->get_info)) gi::detail::method_wrapper<self, Gio::DBusInterfaceInfo_Ref (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_info_>;
  if (init_data.get_properties) methods->get_properties = (decltype (methods->get_properties)) gi::detail::method_wrapper<self, GLib::Variant (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_properties_>;
  if (init_data.get_vtable) methods->get_vtable = (decltype (methods->get_vtable)) gi::detail::method_wrapper<self, Gio::DBusInterfaceVTable_Ref (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_vtable_>;
}
