...
void MountOperationClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.aborted) methods->aborted = (decltype (methods->aborted)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::aborted_>;
  if (init_data.ask_password) methods->ask_password = (decltype (methods->ask_password)) gi::detail::method_wrapper<self, void (*) (const gi::cstring_v message, const gi::cstring_v default_user, const gi::cstring_v default_domain, Gio::AskPasswordFlags flags), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::ask_password_>;
  if (init_data.reply) methods->reply = (decltype (methods->reply)) gi::detail::method_wrapper<self, void (*) (Gio::MountOperationResult result), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::reply_>;
  if (init_data.show_unmount_progress) methods->show_unmount_progress = (decltype (methods->show_unmount_progress)) gi::detail::method_wrapper<self, void (*) (const gi::cstring_v message, gint64 time_left, gint64 bytes_left), gi::transfer_full_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_none_t>>::wrapper<&self::show_unmount_progress_>;
}
