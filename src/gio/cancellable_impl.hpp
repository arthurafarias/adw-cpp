...
void CancellableClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.cancelled) methods->cancelled = (decltype (methods->cancelled)) gi::detail::method_wrapper<self, void (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::cancelled_>;
}
