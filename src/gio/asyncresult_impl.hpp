...
void AsyncResultIfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.get_source_object) methods->get_source_object = (decltype (methods->get_source_object)) gi::detail::method_wrapper<self, GObject::Object (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_source_object_>;
  if (init_data.get_user_data) methods->get_user_data = (decltype (methods->get_user_data)) gi::detail::method_wrapper<self, gpointer (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::get_user_data_>;
  if (init_data.is_tagged) methods->is_tagged = (decltype (methods->is_tagged)) gi::detail::method_wrapper<self, bool (*) (void* source_tag), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::is_tagged_>;
}
