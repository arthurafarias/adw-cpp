...
void SocketAddressClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_family) methods->get_family = (decltype (methods->get_family)) gi::detail::method_wrapper<self, Gio::SocketFamily (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_family_>;
  if (init_data.get_native_size) methods->get_native_size = (decltype (methods->get_native_size)) gi::detail::method_wrapper<self, gssize (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_native_size_>;
  if (init_data.to_native) methods->to_native = (decltype (methods->to_native)) gi::detail::method_wrapper<self, bool (*) (void* dest, gsize destlen, GLib::Error * _error), gi::transfer_none_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_full_t>>::wrapper<&self::to_native_>;
}
