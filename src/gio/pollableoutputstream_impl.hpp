...
void PollableOutputStreamInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.can_poll) methods->can_poll = (decltype (methods->can_poll)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::can_poll_>;
  if (init_data.create_source) methods->create_source = (decltype (methods->create_source)) gi::detail::method_wrapper<self, GLib::Source (*) (Gio::Cancellable cancellable), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::create_source_>;
  if (init_data.is_writable) methods->is_writable = (decltype (methods->is_writable)) gi::detail::method_wrapper<self, bool (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::is_writable_>;
  if (init_data.write_nonblocking) methods->write_nonblocking = (decltype (methods->write_nonblocking)) gi::detail::method_wrapper<self, gssize (*) (const guint8 * buffer, gsize count, GLib::Error * _error), gi::transfer_none_t, std::tuple<gi::transfer_none_t, gi::transfer_none_t, gi::transfer_full_t>>::wrapper<&self::write_nonblocking_>;
}
