      ...
      GObjectConstructParam *construct_properties)
  {
    ...
    try {
      ...
      if (floating && !g_object_is_floating(instance)) {
        ...
        // theoretically not MT safe, but worse things can happen otherwise
        // whoever (some wrapper?) un-floated the object presumably intended
        // to take a (real) ref/ownership, so let's arrange for that here
        // and restore the original floating situation
        g_warning("re-floating instance and converting to ref");
...
public:
  signal(ObjectClass *owner, const gi::cstring_v name,
      ...
      // do not take an extra ref;
      // avoid ref loop and accidental sink of floating ref
      : super(gi::wrap(owner->gobj_(), transfer_full), name)
  {
  ...
  ~signal()
  {
    // clear link (since no real ref was taken at construction)
    this->object_.release_();
  }
};
